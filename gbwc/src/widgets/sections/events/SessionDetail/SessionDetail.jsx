...
export default function SessionDetail({ session, lang, labels }) {
  // Handle both already-resolved values (from API mapping) and lang-object format (from static data)
  function resolve(val) {
    if (!val) return val
    if (Array.isArray(val) || typeof val !== 'object') return val
    return val[lang] ?? val.ru ?? null
  }

  const allPeople = [...(session.members || []), ...(session.guests || [])]
  const t = resolve(session.topics) || []
  const r = session.results ? resolve(session.results) : null
        ...
        <div className="ev2-session-head">
          <div className="ev2-session-meta-row">
            ...
            <div className="ev2-session-meta-info">
              <div className="ev2-session-date">{resolve(session.date)}</div>
              <div className="ev2-session-loc">{resolve(session.location)}</div>
              {(resolve(session.context) !== resolve(session.location)) && (
                <div className="ev2-session-context">{resolve(session.context)}</div>
              )}
            </div>
          </div>
          <div className="ev2-session-theme-block">
            <h2 className="ev2-session-theme">{resolve(session.theme)}</h2>
            <p className="ev2-session-desc">{resolve(session.desc)}</p>
          </div>
        </div>
...
function PersonPortrait({ person, lang }) {
  const initials = person.name ? person.name.split(' ').slice(0, 2).map(w => w[0]).join('') : ''
  function resolve(val) {
    if (!val) return val
    if (Array.isArray(val) || typeof val !== 'object') return val
    return val[lang] ?? val.ru ?? null
  ...
  return (
    <div className="ev2-portrait">
      ...
      <div className="ev2-portrait-info">
        ...
        <div className="ev2-portrait-title">{resolve(person.title)}</div>
      </div>
    </div>
  )
}
