...
export default function InitiativeCard({ item, idx, tasksLabel, lang }) {
  const rev = idx % 2 === 1
  const visual = VISUALS[item.id] || <GenericVisual num={item.num} tag={item.tag} />

  // Helper: safely extract string from plain string or {ru,en,kk} i18n object
  const t = (val) => {
    if (!val) return ''
  ...
  return (
    <div id={`initiative-${item.id}`} className={`init2-block${rev ? ' init2-block--reversed' : ''}`}>
      ...
      <div className="init2-text-side">
        ...
        <h2 className="init2-text-title">{t(item.title)}</h2>
        <div className="init2-ornament"><span className="init2-ornament-gem" /></div>
        <p className="init2-text-desc">{t(item.desc)}</p>
        {item.details && <p className="init2-text-details">{t(item.details)}</p>}
          ...
          <ul className="init2-tasks-list">
            {item.tasks.map((task, i) => (
              <li key={i} className="init2-task-item">
                <span className="init2-task-dot" />{t(task.text ?? task)}
              </li>
            ))}
          </ul>
        </div>
