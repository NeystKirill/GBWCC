...
export default function HomeQuote({ data }) {
  const { lang, resolveCMS, resolveWithFallback } = useLang()
  const fallback = DATA[lang] || DATA.ru
      ...
      <div className="hm-inner">
         ...
         <MotionSection animation="fadeUp" className="hm-left">
           <div className="hm-eyebrow">
             ...
             {resolveWithFallback(data?.eyebrow, fallback.eyebrow)}
           </div>
           <h2 className="hm-title">{resolveWithFallback(data?.title, fallback.title)}</h2>
           <p className="hm-lead">{resolveWithFallback(data?.lead, fallback.lead)}</p>
 
           {/* Quote */}
           <blockquote className="hm-quote">
             <span className="hm-qmark">"</span>
             <p>{resolveWithFallback(data?.quote, fallback.quote)}</p>
             <footer>
               <span className="hm-author-line"/>
               <div>
                 <div className="hm-author">{resolveWithFallback(data?.author, fallback.author)}</div>
                 <div className="hm-role">{resolveWithFallback(data?.role, fallback.role)}</div>
               </div>
             </footer>
           </blockquote>
         </MotionSection>
         ...
         <MotionSection animation="fadeUp" delay={0.15} className="hm-right">
           <div className="hm-pillars">
             {((data?.pillars && data.pillars.length > 0) ? data.pillars : fallback.pillars).map((p, i) => (
               <div key={i} className="hm-pillar">
                 <div className="hm-pillar-icon">{p.icon}</div>
                 <div className="hm-pillar-body">
                   <div className="hm-pillar-label">{resolveCMS(p.label)}</div>
                   <p className="hm-pillar-text">{resolveCMS(p.text)}</p>
                 </div>
               </div>
             ))}
           </div>
