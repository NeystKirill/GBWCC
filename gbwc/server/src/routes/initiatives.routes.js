...
import * as svc from '../services/initiatives.service.js'
...
router.post('/:id/gallery', authenticate, requireAdmin, uploadTo('initiatives').array('files', 20), async (req, res) => {
  const initiative = await svc.getById(req.params.id)
  if (!initiative) return res.status(404).json({ error: 'Initiative not found' })
  const gallery = initiative.gallery || []
  for (const file of req.files) {
    await processImage(file.path)
    gallery.push({ url: getUploadUrl('initiatives', file.filename), caption: {} })
  }
  res.json(await svc.update(req.params.id, { gallery }))
})
