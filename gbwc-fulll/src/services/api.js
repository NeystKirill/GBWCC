 ...
 *
 * Security:
 * - Access token stored in memory (not localStorage) — XSS-resistant
 * - Refresh token in localStorage (fallback until httpOnly cookie available)
 * - Explicit Content-Type on all mutation requests
...
// ─── Token storage ────────────────────────────────────────────────────────────
// Access token: in-memory ONLY (XSS cannot read this)
let _accessToken = null

// Refresh token: localStorage (needed across page reloads)
const getRefreshToken  = () => localStorage.getItem('gbwc_rt')
const setRefreshToken  = (t) => localStorage.setItem('gbwc_rt', t)
const clearRefreshToken = () => localStorage.removeItem('gbwc_rt')

export const tokenStore = {
  set:   (access, refresh) => { _accessToken = access; if (refresh) setRefreshToken(refresh) },
  clear: ()                => { _accessToken = null; clearRefreshToken() },
...
async function _buildError(res) {
  ...
  return Object.assign(new Error(msg), { status: res.status, reqId: body.reqId })
}
...
export const partners    = { ...crud('/partners'),
  list: (type) => apiFetch(`/partners${type ? `?type=${type}` : ''}`),
}
...
export const upload = {
  ...
  multiple: (files, folder = 'gallery') => {
    const form = new FormData()
    files.forEach(f => form.append('files', f))
    form.append('folder', folder)
    return apiFetch('/upload/multiple', { method: 'POST', body: form })
  },
  list:   (q = {})  => { const s = new URLSearchParams(q).toString(); return apiFetch(`/upload${s ? '?' + s : ''}`) },
  remove: (id)      => apiFetch(`/upload/${id}`, { method: 'DELETE' }),
}

export default { auth, pages, leaders, sessions, initiatives, partners, news, media, contact, upload }
