...
function PersonCard({ person, lang }) {
  const title = person.title[lang] || person.title.ru
  ...
  return (
    <div className="hkp-card">
      ...
      <div className="hkp-info">
        <div className="hkp-name">{person.name}</div>
        <div className="hkp-role" style={{color: TYPE_ACCENT[person.type]}}>{title}</div>
        <div className="hkp-org">{person.org}</div>
      </div>
    </div>
  )
}

export default function HomeKeyPeople({ lang }) {
      ...
      <div className="hkp-inner">
        ...
        <MotionSection animation="fadeUp" delay={0.1}>
          <div className="hkp-grid">
            {PEOPLE.map((p,i) => <PersonCard key={i} person={p} lang={lang}/>)}
          </div>
        </MotionSection>
