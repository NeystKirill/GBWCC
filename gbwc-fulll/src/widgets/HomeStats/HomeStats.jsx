...
export default function HomeStats({ lang }) {
  const stats = STATS[lang] || STATS.ru
  return (
    <>
          ...
          {stats.map((s, i) => (
            <MotionSection key={i} animation="fadeUp" delay={i * 0.08} className="hs-card">
              ...
              <div className="hs-card-num">
                <AnimNum to={s.n} sx={s.sx} duration={1800} delay={i * 120} />
              </div>
              {/* lyt-card__meta pattern */}
              <div className="hs-card-label">{s.label}</div>
